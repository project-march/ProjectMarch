...
void ModelPredictiveController::init()
{
  ...
  for (int i = 0; i < ACADO_N; ++i)
  {
    ...
    // Initialize reference with zero
...
void ModelPredictiveController::setReferenceI(int i, vector<double> reference)
{
//  assert(ACADO_NY == reference.size());

  for (int j = 0; j < reference.size(); ++j) {
    acadoVariables.y[i * ACADO_NY + j] = reference[j];
  }
}

void ModelPredictiveController::setReferenceEnd(vector<double> referenceEnd)
{
//  assert(ACADO_NYN == referenceEnd.size());

  for (int j = 0; j < referenceEnd.size(); ++j) {
    acadoVariables.yN[j] = referenceEnd[j];
  }
}
