...
class Setpoint(object):
    ...
    def get_foot_pos_from_angles(setpoint_dic, velocity=False):
        ...
        left_y = - cos(l_haa) * ph - sin(l_haa) * haa_to_foot_length - hip / 2.0
        left_z = - sin(l_haa) * ph + cos(l_haa) * haa_to_foot_length
        left_x = bb + sin(l_hfe) + sin(l_hfe - l_kfe)

        right_y = cos(r_haa) * ph + sin(r_haa) * haa_to_foot_length + hip / 2.0
        right_z = - sin(r_haa) * ph + cos(r_haa) * haa_to_foot_length
    ...
    def get_angles_from_pos(position, foot):
        ...
        # first find the haa angle, this can be done by looking at the y-z plane with the knowledge that there must be
        # a straight line from the foot position tangent to the circle with r = ph around the origin
        if pos_y != 0:
