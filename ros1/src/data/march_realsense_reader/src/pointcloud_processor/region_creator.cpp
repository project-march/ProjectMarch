...
{
    region_vector_ = boost::make_unique<RegionVector>();
...
bool RegionGrower::recursiveRegionGrower(
    const boost::unique_ptr<RegionVector>& last_region_vector,
    ...
    const Normals::Ptr& last_pointcloud_normals, const float& last_tolerance)
{
    ...
    bool success = true;
    boost::unique_ptr<RegionVector> too_small_regions
        = boost::make_unique<RegionVector>();
    boost::unique_ptr<RegionVector> too_large_regions
        = boost::make_unique<RegionVector>();
    boost::unique_ptr<RegionVector> right_size_regions
        = boost::make_unique<RegionVector>();
...
bool RegionGrower::processInvalidRegions(const float& next_tolerance,
    ...
    const boost::unique_ptr<RegionVector>& invalid_regions,
    const PointCloud::Ptr& last_pointcloud,
    const Normals::Ptr& last_pointcloud_normals)
{
    if (invalid_pointcloud->size() > min_desired_cluster_size) {
        ...
        boost::unique_ptr<RegionVector> potential_region_vector
            = boost::make_unique<RegionVector>();
...
void RegionGrower::addRegionsToPointAndNormalVectors(
    const boost::unique_ptr<RegionVector>& right_size_regions,
...
void RegionGrower::fillInvalidClouds(
    const boost::unique_ptr<RegionVector>& invalid_region_vector,
    const PointCloud::Ptr& last_pointcloud,
    const Normals::Ptr& last_pointcloud_normals,
...
void RegionGrower::segmentRegionVector(
    const boost::unique_ptr<RegionVector>& region_vector,
    boost::unique_ptr<RegionVector>& too_small_regions,
    boost::unique_ptr<RegionVector>& too_large_regions,
    boost::unique_ptr<RegionVector>& right_size_regions)
{
...
bool RegionGrower::getRegionVectorFromTolerance(
    const PointCloud::Ptr& pointcloud, const Normals::Ptr& pointcloud_normals,
    const float& tolerance, boost::unique_ptr<RegionVector>& region_vector)
{
