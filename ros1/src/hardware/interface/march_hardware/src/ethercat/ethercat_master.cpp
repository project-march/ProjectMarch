...
void EthercatMaster::ethercatMasterInitiation()
{
    ...
    if (!ec_init(this->if_name_.c_str())) {
        throw error::HardwareException(
            error::ErrorType::NO_SOCKET_CONNECTION, "No socket connection on %s", this->if_name_.c_str());
    }
...
bool EthercatMaster::sendReceivePdo()
{
    if (this->latest_lost_slave_ == -1) {
        ...
        if (wkc < this->expected_working_counter_) {
            ROS_WARN_THROTTLE(
                1, "Working counter: %d  is lower than expected: %d", wkc, this->expected_working_counter_);
            return false;
        }
        return true;
    }
