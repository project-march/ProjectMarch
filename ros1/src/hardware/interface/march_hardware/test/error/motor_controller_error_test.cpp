...
{
  ASSERT_EQ(march::error::parseError((uint16_t) 0, march::error::ErrorRegister::IMOTIONCUBE_MOTION_ERROR), "");
}
...
TEST(TestDetailedMotionError, ParseMultipleDetailedErrors)
{
  ...
  ASSERT_EQ(march::error::parseError(error, march::error::ErrorRegister::IMOTIONCUBE_DETAILED_MOTION_ERROR), expected);
}

TEST(TestSecondDetailedMotionError, ParseNoSecondDetailedMotionError)
{
  ASSERT_EQ(march::error::parseError((uint16_t) 0, march::error::ErrorRegister::IMOTIONCUBE_SECOND_DETAILED_MOTION_ERROR), "");
}
...
TEST(TestSecondDetailedMotionError, ParseMultipleSecondDetailedErrors)
{
  ...
  ASSERT_EQ(march::error::parseError(error, march::error::ErrorRegister::IMOTIONCUBE_SECOND_DETAILED_MOTION_ERROR), expected);
}