...
bool HullParameterDeterminer::getRampSlope()
{
    ...
    HullParameterDeterminer::initializeMarkerListWithId(int id)
    {
    ...
    void HullParameterDeterminer::addDebugMarkersToArray()
    {
        ...
        debug_marker_array.markers.push_back(
    ...
    bool HullParameterDeterminer::getGaitParametersFromSitHeight()
    {
        if (sit_height > min_sit_height && sit_height < max_sit_height) {
            gait_parameters_->first_parameter = (sit_height - min_sit_height)
                / (max_sit_height - min_sit_height);
        } else {
            gait_parameters_->first_parameter = -1;
    ...
    bool HullParameterDeterminer::getSitHeight()
    {
        ...
        success &= cropCloudToHullVectorUnique(
            sit_grid, potential_exo_support_points);
    ...
    bool HullParameterDeterminer::fillSitGrid(PointCloud2D::Ptr & sit_grid)
    {
        if (sit_grid_size < EPSILON) {
            ROS_ERROR_STREAM(
                "The grid size of the sit grid is too close to zero. "
                "Current value is "
                << sit_grid_size << " but should be larger then " << EPSILON);
            return false;
        }
        int x_points
            = int(round((max_x_search_sit - min_x_search_sit) / sit_grid_size))
            + 1;
        ...
        for (int x_index = 0; x_index < x_points; ++x_index) {
            for (int y_index = 0; y_index < y_points; ++y_index) {
                ...
                grid_point.x
                    = float(x_index) * sit_grid_size + min_x_search_sit;
        ...
        const pcl::Normal& normal, float& slope)
    {
        ...
        if (!linear_algebra_utilities::normalizeNormal(
    ...
    bool HullParameterDeterminer::getPossibleMostDesirableLocation()
    {
        ...
        for (pcl::PointNormal& possible_foot_location :
            *possible_foot_locations) {
            ...
            success &= getDistanceToObject(
                possible_foot_location, distance_to_object);
        ...
        pcl::PointNormal possible_foot_location, double& distance)
    {
        switch (realsense_category_.value()) {
            ...
            default: {
                ROS_ERROR_STREAM(
                    "getDistanceToObject method is not implemented "
                    "for selected obstacle "
                    << realsense_category_.value());
        ...
        pcl::PointNormal possible_foot_location)
    {
        ...
        success &= (potential_foot_support_cloud->size()
        ...
        pcl::PointNormal possible_foot_location)
    {
        ...
        foot_pointcloud->points[0].x
            = possible_foot_location.x + foot_length_back;
        foot_pointcloud->points[0].y
            = possible_foot_location.y - foot_width / 2.0F;

        foot_pointcloud->points[1].x
            = possible_foot_location.x + foot_length_back;
        foot_pointcloud->points[1].y
            = possible_foot_location.y + foot_width / 2.0F;

        // Deviation front is subtracted as the forward direction of the
        // exoskeleton is the negative x direction in the simulation
        foot_pointcloud->points[2].x
            = possible_foot_location.x - foot_length_front;
        foot_pointcloud->points[2].y
            = possible_foot_location.y - foot_width / 2.0F;

        foot_pointcloud->points[3].x
            = possible_foot_location.x - foot_length_front;
        foot_pointcloud->points[3].y
            = possible_foot_location.y + foot_width / 2.0F;
    }
    ...
    bool HullParameterDeterminer::getGeneralMostDesirableLocation()
    {
        if (general_most_desirable_location_is_mid) {
            most_desirable_foot_location_.x
                = (min_x_stairs + max_x_stairs) / 2.0F;
            most_desirable_foot_location_.y = y_location;
            most_desirable_foot_location_.z
                = (min_z_stairs + max_z_stairs) / 2.0F;
        ...
        const PointNormalCloud::Ptr& output_cloud)
    {
        ...
        bool success = true;
        for (int hull_index = 0; hull_index < hull_vector_->size();
             hull_index++) {
            ...
            success &= cropCloudToHull(elevated_cloud,
                hull_vector_->at(hull_index), polygon_vector_->at(hull_index));
