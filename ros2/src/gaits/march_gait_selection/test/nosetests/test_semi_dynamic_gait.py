...
class TestSemiDynamicGaitSelection(unittest.TestCase):
    ...
    def test_can_freeze_after_sec(self):
        self.semi_dynamic_gait.update(CustomDuration(seconds=2))
    ...
    def test_subgait_starts_from_0_after_freeze(self):
        self.semi_dynamic_gait.update(CustomDuration(seconds=2))
        self.semi_dynamic_gait.freeze(CustomDuration(seconds=2))
        self.semi_dynamic_gait.update(CustomDuration(seconds=0.5))
        self.assertEqual(self.semi_dynamic_gait._time_since_start, CustomDuration(seconds=0))

    def test_freeze_duration(self):
        self.semi_dynamic_gait.update(CustomDuration(seconds=2))
        self.semi_dynamic_gait.freeze(CustomDuration(seconds=10))
        self.semi_dynamic_gait.update(CustomDuration(seconds=1))
        self.semi_dynamic_gait.update(CustomDuration(seconds=9.5))
        self.assertEqual(self.semi_dynamic_gait._time_since_start, CustomDuration(seconds=9.5))

    def test_freeze_duration_done(self):
        self.semi_dynamic_gait.update(CustomDuration(seconds=2))
        self.semi_dynamic_gait.freeze(CustomDuration(seconds=10))
        self.semi_dynamic_gait.update(CustomDuration(seconds=1))
        self.semi_dynamic_gait.update(CustomDuration(seconds=11))
        self.assertEqual(self.semi_dynamic_gait._time_since_start, CustomDuration(seconds=0))

    def test_position_after_time_begin(self):
        self.semi_dynamic_gait.update(CustomDuration(seconds=1))
        self.assertEqual(
            self.semi_dynamic_gait._position_after_time(CustomDuration(seconds=0)),
    ...
    def test_position_after_time_end(self):
        self.semi_dynamic_gait.update(CustomDuration(seconds=2))
        self.assertEqual(
            self.semi_dynamic_gait._position_after_time(CustomDuration(seconds=2.75)),
            {
                "left_ankle": 0.0436,
                "left_hip_aa": 0.0,
                "left_hip_fe": -0.1396,
