...
class PressureSoleVis(Node):
    ...
    def __init__(self):
        ...
        self.subscription = self.create_subscription(
            PressureSolesData, "/march/pressure_sole_data", self.listener_callback, 10
        )
        ...
        self.x_coord_left = np.array(
            [0.044, 0.044, 0.139, 0.206, 0.219, 0.228, 0.265, 0.275]
        )  # the coordinates of the pressure sole pads
    ...
    def create_heatmap(self, data):
        ...
        for i in range(int(len(data) / 2)):
            ...
            for j in range(
                int(effective_data[i + 8] * resolution)
            ):  # something that creates an amount of markers based on the height
                point_container = Point()
                color = ColorRGBA()
                color.a = 1.0
                normalized_z = (
                    (effective_data[i + 8] - self.min_z) / (self.max_z - self.min_z)
                ...
                point_container.z = (
                    j * marker_container_strip.scale.z
                )  # the scale is from the middle of the point so move the point up

                color.r, color.g, color.b = self.hsv_to_rgb(
                    (normalized_z / (int(effective_data[i + 8] * resolution)) * j), 1.0, 1.0
                )
            ...
            for j in range(
                int(effective_data[i] * resolution)
            ):  # something that creates an amount of markers based on the height
                point_container = Point()
                color = ColorRGBA()
                color.a = 1.0
                normalized_z = (
                    (effective_data[i] - self.min_z) / (self.max_z - self.min_z) if self.max_z != self.min_z else 0.2
                )
                ...
                color.r, color.g, color.b = self.hsv_to_rgb(
                    (normalized_z / (int(effective_data[i] * resolution)) * j), 1.0, 1.0
                )
