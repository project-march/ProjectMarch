...
AbsoluteEncoder::AbsoluteEncoder(size_t counts_per_rotation, MotorControllerType motor_controller_type,
    Direction direction, int32_t lower_limit_iu, int32_t upper_limit_iu, double lower_limit_rad, double upper_limit_rad,
...
AbsoluteEncoder::AbsoluteEncoder(size_t counts_per_rotation, MotorControllerType motor_controller_type,
    int32_t lower_limit_iu, int32_t upper_limit_iu, double lower_limit_rad, double upper_limit_rad,
    double lower_soft_limit_rad, double upper_soft_limit_rad)
    : AbsoluteEncoder(counts_per_rotation, motor_controller_type, Direction::Positive, lower_limit_iu, upper_limit_iu,
        lower_limit_rad, upper_limit_rad, lower_soft_limit_rad, upper_soft_limit_rad)
{
}

void AbsoluteEncoder::checkRangeOfMotion(double lower_limit_rad, double upper_limit_rad)
{
    if (lower_limit_iu_ >= upper_limit_iu_ || lower_soft_limit_iu_ >= upper_soft_limit_iu_
        || lower_soft_limit_iu_ < lower_limit_iu_ || upper_soft_limit_iu_ > upper_limit_iu_) {
        throw error::HardwareException(error::ErrorType::INVALID_RANGE_OF_MOTION,
            ...
            lower_soft_limit_iu_, upper_soft_limit_iu_, lower_limit_iu_, upper_limit_iu_);
    }

    const double range_of_motion = upper_limit_rad - lower_limit_rad;
    const double encoder_range_of_motion = positionIUToRadians(upper_limit_iu_) - positionIUToRadians(lower_limit_iu_);
