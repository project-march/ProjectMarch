#ifndef MARCH_CAMERAS_INTERFACE_H
#define MARCH_CAMERAS_INTERFACE_H

// TODO: Change the march_shared_msgs message for the current state
...
public:
    ...
    explicit CamerasInterface(rclcpp::Node* n, const std::string& left_or_right);
    void readParameters(const std::vector<rclcpp::Parameter>& parameters);
    ~CamerasInterface() = default;
