...
{
    service_ = this->create_service<march_shared_msgs::srv::GetCurrentJointPositions>(
...
void CurrentJointPositionServer::handle_request(const std::shared_ptr<march_shared_msgs::srv::GetCurrentJointPositions::Request> request,
        std::shared_ptr<march_shared_msgs::srv::GetCurrentJointPositions::Response> response)
{
    ...
    std::vector<double> current_joint_positions_vector(
        current_joint_positions.data(), 
        current_joint_positions.data() + current_joint_positions.size());
    ...
    std::vector<double> current_joint_velocities_vector(
        ...
        current_joint_velocities.data() + current_joint_velocities.size());
