...
def data_files():
    ...
    ros1_source = '../../../../ros1/src/gaits/march_gait_selection/test/testing_gait_files'
    data = [
        (os.path.join('share', 'ament_index', 'resource_index', 'packages'),
         [os.path.join('resource', package_name)]),
        (os.path.join('share', package_name), ['package.xml']),
        (os.path.join('share', package_name, 'test', 'resources'),
         [os.path.join(ros1_source, 'default.yaml')]),
        (os.path.join('share', package_name, 'launch'), glob('launch/*.launch.py'))
    ]
    for file in iglob(os.path.join(ros1_source, '**', '*.subgait'), recursive=True):
        data.append((os.path.join('share', package_name, 'test', 'resources',
                                  os.path.dirname(os.path.relpath(file, ros1_source))), [file]))
    for file in iglob(os.path.join(ros1_source, '**', '*.gait'), recursive=True):
        data.append((os.path.join('share', package_name, 'test', 'resources',
                                  os.path.dirname(os.path.relpath(file, ros1_source))), [file]))
    return data
