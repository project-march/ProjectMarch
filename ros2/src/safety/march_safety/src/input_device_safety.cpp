...
void InputDeviceSafety::update()
{
    if (last_alive_stamps_.empty()) {
        ...
        RCLCPP_INFO_THROTTLE(
            node_->get_logger(), *node_->get_clock(), THROTTLE_DURATION_MS, "No input device connected yet");
        return;
    }
    ...
    if (!has_connections) {
        RCLCPP_INFO_SKIPFIRST_THROTTLE(
            node_->get_logger(), *node_->get_clock(), THROTTLE_DURATION_MS, "No input device connected");
    }
}
...
void InputDeviceSafety::check_last_alive_stamp(const std::string& id, const rclcpp::Time& last_alive)
{
    ...
    if (is_connected && timed_out) {
        ...
        RCLCPP_INFO_STREAM(node_->get_logger(),
            "Input device `" << id << "` reconnected. Total connected is " << connected_devices_.size());
    }
    ...
    if (now + rclcpp::Duration(/*duration=*/0.5s) < last_alive) {
        RCLCPP_WARN_STREAM_THROTTLE(node_->get_logger(), *node_->get_clock(), THROTTLE_DURATION_MS,
            "Input device `" << id << "` alive message is from the future. Current time is " << now.seconds()
                             << " and last alive message was " << last_alive.seconds());
    }
}
