    ...
    , m_left_or_right(left_or_right)
{
    ...
    if (m_left_or_right == "left") {
        m_switch_factor = -1;
        m_serial_number = "944622074337";
    } else {
        m_switch_factor = 1;
        m_serial_number = "944622071535";
    }
...
void CameraInterface::processPointCloud(const PointCloud::Ptr& pointcloud) {
    ...
    while (m_locked) {
        rclcpp::sleep_for(std::chrono::nanoseconds(10000000)); // 10 ms
    }
    m_locked = true;
    m_last_frame_time = std::clock();
    m_frame_wait_counter = 0;
...
bool CameraInterface::updateTransformations(const ros::Time& timeStamp) {

  const Parameters parameters{parameters_.getData()};
  try {

    tf::StampedTransform transformTf;
    m_tf_listener.lookupTransform(robotBaseFrameId_, sensorFrameId_, timeStamp,
...
bool SensorProcessorBase::transformPointCloud(PointCloudType::ConstPtr pointCloud, PointCloudType::Ptr pointCloudTransformed,
  ...
  try {
    m_tf_listener.waitForTransform(targetFrame, inputFrameId, timeStamp, ros2::Duration(1.0), ros2::Duration(0.001));
    m_tf_listener.lookupTransform(targetFrame, inputFrameId, timeStamp, transformTf);
...
void CameraInterface::removePointsOutsideLimits(const PointCloud::Ptr& point_cloud) {
    ...
    pass_through_filter.setFilterLimits(lowerThreshold, upperThreshold);
