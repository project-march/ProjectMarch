...
TEST_F(JointTest, TestReadEncodersRecalibratePosition)
{
  ...
  double absolute_noise = -2 * this->imc->getAbsoluteRadPerBit() - this->imc->getIncrementalRadPerBit();
...
TEST_F(JointTest, TestReadEncodersIncrementalOutlier)
{
  ros::Duration elapsed_time(0.2);
  double velocity = 0.5;
  double incremental_noise = 3 * this->imc->getAbsoluteRadPerBit() + 2 * this->imc->getIncrementalRadPerBit();
