...
//  min_cluster_size = yaml_utilities::grabParameter<int>(config_tree, "min_cluster_size")
}
void regionGrower::setup_region_grower()
{
//  ROS_INFO_STREAM("params: " << number_of_neighbours << min_cluster_size << max_cluster_size << smoothness_threshold << curvature_threshold);
//  ROS_INFO_STREAM("start setup");
  pcl::search::Search<pcl::PointXYZ>::Ptr tree (new pcl::search::KdTree<pcl::PointXYZ>);
  region_grower.setMinClusterSize(min_cluster_size);
  region_grower.setMaxClusterSize(max_cluster_size);
//  ROS_INFO_STREAM("setup 2");
...
//  ROS_INFO_STREAM("setup 3");
  region_grower.setSmoothnessThreshold(smoothness_threshold);
  region_grower.setCurvatureThreshold(curvature_threshold);
  ROS_INFO_STREAM("end setup");
//  bool test = region_grower->prepareForSegmentation();
//  ROS_INFO("Ready to grow regions: %i", test);
}
bool regionGrower::extract_regions()
{
  region_grower.extract(*regions_vector_);
//  ROS_INFO_STREAM("Total numbers of clusters found: " << regions_vector_->size());
  if (debugging_)
  {
