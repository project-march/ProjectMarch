...
bool MarchPdbStateController::init(MarchPdbStateInterface* hw,
    ros::NodeHandle& root_nh, ros::NodeHandle& controller_nh)
{
    ...
    if (pdb_state_names.size() == 1) {
        ...
        // realtime publisher
        this->rt_pub_ = std::make_unique<realtime_tools::RealtimePublisher<
...
MarchPdbStateController::createHighVoltageNetsMessage(
    march::HighVoltage high_voltage)
{
...
MarchPdbStateController::createLowVoltageNetsMessage(
    march::LowVoltage low_voltage)
{
...
void MarchPdbStateController::update(
    const ros::Time& time, const ros::Duration& /*period*/)
{
    // limit rate of publishing
    if (this->publish_rate_ > 0.0
        && this->last_publish_times_ + ros::Duration(1.0 / this->publish_rate_)
            < time) {
        // try to publish
        if (this->rt_pub_->trylock()) {
            // we're actually publishing, so increment time
            this->last_publish_times_ = this->last_publish_times_
                + ros::Duration(1.0 / this->publish_rate_);
            march::PowerDistributionBoard* pdb
                = this->pdb_state_.getPowerDistributionBoard();
            this->rt_pub_->msg_.header.stamp = ros::Time::now();
            this->rt_pub_->msg_.low_voltage_nets
                = createLowVoltageNetsMessage(pdb->getLowVoltage());
            this->rt_pub_->msg_.high_voltage_nets
                = createHighVoltageNetsMessage(pdb->getHighVoltage());
            this->rt_pub_->msg_.master_shutdown_requested
                = pdb->getMasterShutdownRequested();
            this->rt_pub_->msg_.power_distribution_board_current
                = pdb->getPowerDistributionBoardCurrent();
            this->rt_pub_->msg_.high_voltage_enabled
                = pdb->getHighVoltage().getHighVoltageEnabled();
