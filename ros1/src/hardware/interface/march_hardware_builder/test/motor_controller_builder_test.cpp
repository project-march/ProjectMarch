...
protected:
  ...
  void SetUp() override
  {
    this->base_path = ros::package::getPath("march_hardware_builder").append("/test/yaml/motor_controller");
...
TEST_F(MotorControllerBuilderTest, ValidIMotionCube)
{
  ...
  auto created = HardwareBuilder::createMotorController(config, this->joint, this->pdo_interface, this->sdo_interface);
