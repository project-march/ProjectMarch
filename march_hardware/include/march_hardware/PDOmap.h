...
public:
  ...
  std::unordered_map<IMCObjectName, uint8_t> map(SdoInterface& sdo, int slave_index, DataDirection direction);
...
private:
  ...
  std::unordered_map<IMCObjectName, uint8_t> configurePDO(SdoInterface& sdo, int slave_index, int base_register,
