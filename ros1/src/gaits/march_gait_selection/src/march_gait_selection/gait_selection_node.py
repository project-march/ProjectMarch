...
from ros1.src.balance.march_moveit_interface.src.march_moveit_interface.moveit_interface import BalanceGait
...
def create_services(gait_selection, gait_state_machine):
    ...
    rospy.Service(
        ...
        PossibleGaits,
        lambda msg: PossibleGaitsResponse(
            gaits=gait_state_machine.get_possible_gaits()
        ),
    )
...
def create_subscribers(gait_state_machine):
    rospy.Subscriber(
        "/march/error", Error, lambda msg: error_cb(gait_state_machine, msg)
    )


def create_publishers(gait_state_machine):
    current_state_pub = rospy.Publisher(
        "/march/gait_selection/current_state", CurrentState, queue_size=10
    )
    current_gait_pub = rospy.Publisher(
        "/march/gait_selection/current_gait", CurrentGait, queue_size=10
    )

    def current_state_cb(state, next_is_idle):
        current_state_pub.publish(
...
def main():
    ...
    rospy.loginfo(
        "Gait selection initialized with package {0} of directory {1}".format(
            gait_package, gait_directory
        )
    )

    if balance_used:
        balance_gait = BalanceGait.create_balance_subgait(
            gait_selection["balance_walk"]
        )
        if balance_gait is not None:
            gait_selection.add_gait(balance_gait)

    scheduler = TrajectoryScheduler(
        "/march/controller/trajectory/follow_joint_trajectory"
    )

    state_input = StateMachineInput()
    gait_state_machine = GaitStateMachine(
        gait_selection, scheduler, state_input, update_rate
    )
    rospy.loginfo("Gait state machine successfully generated")

    rospy.core.add_preshutdown_hook(
        lambda reason: gait_state_machine.request_shutdown()
    )
