...
bool IMotionCube::writeInitialSettings(SdoSlaveInterface& sdo, int cycle_time)
{
  ...
  */
    ...
    int min_pos_lim = sdo.write(/*index=*/0x607D, /*sub=*/1, this->getAbsoluteEncoder()->getLowerSoftLimitIU());

    // position limit -- max position
    int max_pos_lim = sdo.write(/*index=*/0x607D, /*sub=*/2, this->getAbsoluteEncoder()->getUpperSoftLimitIU());
    ...
    int abort_con = sdo.write<int16_t>(/*index=*/0x6007, /*sub=*/0, /*value=*/1);

    // set the ethercat rate of encoder in form x*10^y
    int rate_ec_x = sdo.write<uint8_t>(/*index=*/0x60C2, /*sub=*/1, cycle_time);
    int rate_ec_y = sdo.write<int8_t>(/*index=*/0x60C2, /*sub=*/2, /*value=*/-3);

    // use filter object to read motor voltage
    int volt_address = sdo.write<int16_t>(/*index=*/0x2108, /*sub=*/1, /*value=*/0x0232);
    int volt_filter = sdo.write<int16_t>(/*index=*/0x2108, /*sub=*/2, /*value=*/32767);
...
void IMotionCube::actuateRadians(float target_position)
{
    if (this->actuation_mode_ != ActuationMode::position) {
        throw error::HardwareException(error::ErrorType::INVALID_ACTUATION_MODE,
            "trying to actuate rad, while actuation mode is %s", this->actuation_mode_.toString().c_str());
    }
    ...
    if (std::abs(target_position - this->getAbsolutePositionUnchecked()) > MAX_TARGET_DIFFERENCE) {
        throw error::HardwareException(error::ErrorType::TARGET_EXCEEDS_MAX_DIFFERENCE,
            ...
            "%d",
            target_position, MAX_TARGET_DIFFERENCE, this->getAbsolutePositionUnchecked(), this->getSlaveIndex());
    }
    this->actuateIU(this->absolute_encoder_->positionRadiansToIU(target_position));
}
...
void IMotionCube::actuateIU(int32_t target_iu)
{
    if (!this->getAbsoluteEncoder()->isValidTargetIU(this->getAbsolutePositionIU(), target_iu)) {
        throw error::HardwareException(error::ErrorType::INVALID_ACTUATE_POSITION,
            ...
            this->absolute_encoder_->getLowerSoftLimitIU(), this->absolute_encoder_->getUpperSoftLimitIU());
    }
...
void IMotionCube::actuateTorque(float target_torque)
{
    ...
    if (this->actuation_mode_ != ActuationMode::torque) {
        throw error::HardwareException(error::ErrorType::INVALID_ACTUATION_MODE,
            "trying to actuate torque, while actuation mode is %s", this->actuation_mode_.toString().c_str());
    }
...
float IMotionCube::getMotorCurrent()
{
    ...
    const float IU_CONVERSION_CONST = 65520.0; // Conversion parameter, see Technosoft CoE programming
    // manual
    ...
    return (2.0F * PEAK_CURRENT / IU_CONVERSION_CONST)
        * static_cast<float>(motor_current_iu); // Conversion to Amp, see Technosoft CoE
    // programming manual
}
...
float IMotionCube::getMotorControllerVoltage()
{
    ...
    return (V_DC_MAX_MEASURABLE / IU_CONVERSION_CONST)
        * static_cast<float>(imc_voltage_iu); // Conversion to Volt, see Technosoft CoE
    // programming manual
}
    ...
    while (!target_state.isReached(this->getStatusWord())) {
        ...
        if (target_state.getState() == IMotionCubeTargetState::OPERATION_ENABLED.getState()
            ...
            ROS_FATAL("Detailed Error (DER): %s",
                error::parseError(this->getDetailedError(), error::ErrorRegister::IMOTIONCUBE_DETAILED_MOTION_ERROR)
                    .c_str());
            ROS_FATAL("Detailed Error 2 (DER2): %s",
                error::parseError(
                    this->getSecondDetailedError(), error::ErrorRegister::IMOTIONCUBE_SECOND_DETAILED_MOTION_ERROR)
                    .c_str());
...
uint16_t IMotionCube::computeSWCheckSum(uint16_t& start_address, uint16_t& end_address)
{
    ...
    throw error::HardwareException(
        error::ErrorType::INVALID_SW_STRING, "The .sw file has no delimiter of type %s", delimiter.c_str());
}
...
bool IMotionCube::verifySetup(SdoSlaveInterface& sdo)
{
    ...
    const int checksum_setup = sdo.write<uint32_t>(
        /*index=*/0x2069, /*sub=*/0, (uint32_t)(end_address << 16U) | start_address);
...
void IMotionCube::downloadSetupToDrive(SdoSlaveInterface& sdo)
{
    ...
    while ((pos = sw_string_.find(delimiter, old_pos)) != std::string::npos) {
        ...
        if (old_pos == 0) {
            const uint16_t mem_location = std::stoi(token, /*__idx=*/nullptr, /*__base=*/16);
            const uint16_t mem_setup = 9; // send 16-bits and auto increment
            result = (uint32_t)sdo.write<uint32_t>(/*index=*/0x2064, /*sub=*/0,
                (uint32_t)(mem_location << 16U) | mem_setup); // write the write-configuration
            final_result |= result;
        } else {
            if (pos == old_pos) {
                break;
                ...
                uint32_t data = 0;
                if (pos != old_pos) {
                    data = ((uint32_t)std::stoi(next_token, /*__idx=*/nullptr, /*__base=*/16) << 16U)
                        | (uint32_t)std::stoi(token, /*__idx=*/nullptr, /*__base=*/16);
                } else {
                    data = (uint32_t)std::stoi(token, /*__idx=*/nullptr, /*__base=*/16);
                }
...
{
    return (float)this->getAbsoluteEncoder()->velocityIUToRadians(getAbsoluteVelocityIU());
}
