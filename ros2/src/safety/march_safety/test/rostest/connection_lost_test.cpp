...
TEST(TestConnectionLost, connectionLost)
{
    ...
    nh.getParam("/march_safety_node/input_device_connection_timeout",
        input_device_connection_timeout);
    double send_errors_interval;
    nh.getParam(
        "/march_safety_node/send_errors_interval", send_errors_interval);

    ros::Publisher pub_alive = nh.advertise<march_shared_resources::Alive>(
        "march/input_device/alive", 0);
    ...
    ros::spinOnce();
    double sleep_ms
        = send_errors_interval * 0.9 + input_device_connection_timeout;
