...
class FootstepPlannerTest : public ::testing::Test {
    ...
    void SetUp(){
        ...
        m_footstep_planner_node = std::make_unique<FootstepPlannerNode>(); 
    }
    std::unique_ptr<FootstepPlanner> m_footstep_planner; 
    std::unique_ptr<FootstepPlannerNode> m_footstep_planner_node; 
};
...
TEST_F(FootstepPlannerTest, test_compare_distance){
    ...
    ASSERT_TRUE(m_footstep_planner_node->compareDistance(plane1, plane2)); 
}
