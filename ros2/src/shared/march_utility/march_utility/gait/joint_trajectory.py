...
class JointTrajectory(object):
    ...
    def from_setpoint_dict(
        cls, name: str, limits: Limits, setpoint_dict: List[dict], duration: CustomDuration
    ) -> JointTrajectory:
        ...
        setpoints = [
            Setpoint(
                time=CustomDuration(nanoseconds=setpoint["time_from_start"]),
    ...
    @property
    def duration(self) -> CustomDuration:
        """Get the duration of the joint trajectory."""
        return self._duration

    def set_duration(self, new_duration: CustomDuration, rescale: bool = True) -> None:
    ...
    def from_begin_point(self, begin_time: CustomDuration, position: float) -> None:
        """
        ...
        """
        begin_time -= CustomDuration(seconds=1)
        for setpoint in reversed(self.setpoints):
        ...
        self.setpoints = [
            Setpoint(time=CustomDuration(), position=position, velocity=0)
        ] + self.setpoints
