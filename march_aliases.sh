...
alias m8_test_rotational='sudo -v && sfox && sros2 &&  ros2 launch march_launch start_test_setup.launch.py test_rotational:=true  robot:=test_joint_rotational rviz:=false gait_directory:=test_joint_rotational_gaits layout:=test_joint realsense:=false control_yaml:=effort_control/test_joint_rotational_control.yaml simulation:=false'
