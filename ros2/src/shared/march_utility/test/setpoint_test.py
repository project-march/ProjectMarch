...
class SetpointTest(unittest.TestCase):
    ...
    def test_find_known_position_forward(self):
        ...
        expected_right_foot = Foot(
            Side.right, Vector3d(r_ul + r_ll + r_hl, base / 2.0 + r_ph, 0), Vector3d(0, 0, 0)
        )
        expected_left_foot = Foot(
            Side.right, Vector3d(l_ul + l_ll + l_hl, -base / 2.0 - l_ph, 0), Vector3d(0, 0, 0)
        )
    ...
    def test_find_known_position_down(self):
        ...
        resulting_state = FeetState.from_setpoint_dict(setpoint_dict)
        expected_right_foot = Foot(
            Side.right, Vector3d(r_hl, base / 2.0 + r_ph, r_ul + r_ll), Vector3d(0, 0, 0)
        )
        expected_left_foot = Foot(
            Side.right, Vector3d(l_hl, -base / 2.0 - l_ph, l_ul + l_ll), Vector3d(0, 0, 0)
        )
        expected_state = FeetState(expected_right_foot, expected_left_foot, Duration())
        self.assertTrue(
            Vector3d.is_close_enough(
    ...
    def test_find_known_position_sit(self):
        ...
        resulting_state = FeetState.from_setpoint_dict(setpoint_dict)
        expected_right_foot = Foot(
            Side.right, Vector3d(r_ul + r_hl, base / 2.0 + r_ph, r_ll), Vector3d(0, 0, 0)
        )
        expected_left_foot = Foot(
            Side.right, Vector3d(l_ul + l_hl, -base / 2.0 - l_ph, l_ll), Vector3d(0, 0, 0)
        )
        expected_state = FeetState(expected_right_foot, expected_left_foot, Duration())
        self.assertTrue(
            Vector3d.is_close_enough(
