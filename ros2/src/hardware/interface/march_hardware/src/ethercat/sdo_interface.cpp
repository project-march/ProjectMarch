    ...
    uint16_t slave, uint16_t index, uint8_t sub, std::size_t size, void* value)
{
    ...
    if (working_counter == 0) {
        logger_->fatal(logger_->fstring("sdo_write: Error occurred when writing: slave %i, reg 0x%X, sub index %i",
                                        slave, index, sub));
    }
    ...
    int& val_size, void* value) const
{
    ...
    if (working_counter == 0) {
        logger_->fatal(logger_->fstring("sdo_read: Error occurred when reading: slave %i, reg 0x%X, sub index %i",
                                        slave, index, sub));
    }
    return working_counter;
}
