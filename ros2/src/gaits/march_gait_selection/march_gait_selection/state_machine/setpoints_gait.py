...
class SetpointsGait(GaitInterface, Gait):
    ...
    def __init__(self, gait_name, subgaits, graph):
        ...
        self._transition_is_delayed = False
    ...
    def update(
        ...
        """
        self._current_time = current_time
        if self._transition_is_delayed:
            self._transition_is_delayed = False
            self._scheduled_early = True
            self._next_subgait = self._current_subgait
            self._update_time_stamps(self._current_subgait, Duration(0.2))
            return GaitUpdate.should_schedule_early(
                TrajectoryCommand.from_subgait(self._current_subgait,
                                               self._current_time + Duration(0.2))
            )
        ...
        if self._current_time >= self._end_time - Duration(0.1):
    ...
    def subgait_failed(self):
        self._transition_is_delayed = True
