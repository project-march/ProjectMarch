...
TEST_F(JointTest, ActuatePosition)
{
  ...
  march::Joint joint("actuate_false", /*net_number=*/0, /*allow_actuation=*/true, std::move(this->imc));
...
{
  march::Joint joint("actuate_false", /*net_number=*/0, /*allow_actuation=*/false, std::move(this->imc));
  EXPECT_FALSE(joint.canActuate());
...
TEST_F(JointTest, ActuateTorque)
{
  ...
  march::Joint joint("actuate_true", /*net_number=*/0, /*allow_actuation=*/true, std::move(this->imc));
...
{
  march::Joint joint("get_temperature", /*net_number=*/0, /*allow_actuation=*/false, std::move(this->imc));
...
TEST_F(JointTest, ResetController)
{
  ...
  march::Joint joint("reset_controller", /*net_number=*/0, /*allow_actuation=*/true, std::move(this->imc));
...
{
  EXPECT_CALL(*this->imc, getIncrementalPositionUnchecked()).WillOnce(Return(/*value=*/5));
  EXPECT_CALL(*this->imc, getAbsolutePositionUnchecked()).WillOnce(Return(/*value=*/3));
