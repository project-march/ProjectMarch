...
from march_utility.exceptions.gait_exceptions import UnknownDimensionsError, WrongRealSenseConfigurationError
...
class InterpolationDimensions(Enum):
    ...
    def from_integer(cls, dimensions: int):
        if dimensions == 1:
            ...
            raise WrongRealSenseConfigurationError(f"The dimensions value is not `1` "
                                                   f"or `2`, but {dimensions}")
