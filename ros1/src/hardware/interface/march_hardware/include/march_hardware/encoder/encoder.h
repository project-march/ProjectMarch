...
public:
    ...
    /**
     * Reads out the encoder from the slave and returns the value in Internal
     * Units (IU).
     * @param pdo PDO interface to use for reading
     * @param byte_offset the byte offset in the slave register for the IU
     * position
    ...
    /**
     * Reads out the encoder from the slave and transforms the result to
     * radians.
     * @param pdo PDO interface to use for reading
     * @param byte_offset the byte offset in the slave register for the IU
     * position
    ...
    /**
     * Reads out the velocity of the encoder from the slave and returns the
     * value in Internal Units per second (IU/s).
     * @param pdo PDO interface to use for reading
     * @param byte_offset the byte offset in the slave register for the IU
     * velocity
     * @returns The current velocity measured by the encoder in Internal Units
     * per second (IU/s)
     */
    double getVelocityIU(
        const PdoSlaveInterface& pdo, uint8_t byte_offset) const;

    /**
     * Reads out the velocity of the encoder from the slave and transforms the
     * result to radians per second.
     * @param pdo PDO interface to use for reading
     * @param byte_offset the byte offset in the slave register for the IU
     * velocity
     * @returns The current velocity measured by the encoder in radians per
     * second
     */
    double getVelocityRad(
        const PdoSlaveInterface& pdo, uint8_t byte_offset) const;
    ...
    // constant used for converting a fixed point 16.16 bit number to a double,
