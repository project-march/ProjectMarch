...
Transformer::Transformer(
    std::shared_ptr<tf2_ros::Buffer> tfBuffer, std::string frame_id_to_transform_to, std::string fixed_frame)
{
    yaw = 0.0;
    pitch = 0.0;
    roll = 0.0;
...
{
    pcl_ros::transformPointCloud(*cloud, *cloud, transform_stamped_msg.transform);
}
...
void Transformer::createTransform()
{
    if (tfBuffer_->canTransform(
            fixed_frame_, frame_id_to_transform_to_, ros::Time(), ros::Duration(/*t=*/1.0), &error_str)) {
        transform_stamped_msg = tfBuffer_->lookupTransform(fixed_frame_, frame_id_to_transform_to_, ros::Time(/*t=*/0));
    } else {
...
{
    transform_stamped_msg.transform.rotation.x = yaw_stamped_msg.transform.rotation.x;
    transform_stamped_msg.transform.rotation.y = yaw_stamped_msg.transform.rotation.y;
    transform_stamped_msg.transform.rotation.z = yaw_stamped_msg.transform.rotation.z;
    transform_stamped_msg.transform.rotation.w = yaw_stamped_msg.transform.rotation.w;
}
