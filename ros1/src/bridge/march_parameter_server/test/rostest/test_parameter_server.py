...
class TestParameterServer(unittest.TestCase):
    ...
    def test_get_parameter(self, param_type, service_type, service_request_type, value):
        ...
        rospy.wait_for_service('/march/parameter_server/get_param_{param_type}'.format(param_type=param_type))

        service = rospy.ServiceProxy('/march/parameter_server/get_param_{param_type}'.format(param_type=param_type),
                                     service_type)
        response = service.call(service_request_type('/test/{param_type}_get'.format(param_type=param_type)))
    ...
    @parameterized.expand([
        ...
        ['string_list', SetParamStringList, SetParamStringListRequest, ['abcde', 'fghijklm']],
    ])
    def test_set_parameter(self, param_type, service_type, service_request_type, value):
        rospy.wait_for_service('/march/parameter_server/set_param_{param_type}'.format(param_type=param_type))

        service = rospy.ServiceProxy('/march/parameter_server/set_param_{param_type}'.format(param_type=param_type),
                                     service_type)
        response = service.call(service_request_type('/test/{param_type}_set'.format(param_type=param_type), value))

        self.assertEqual(rospy.get_param('/test/{param_type}_set'.format(param_type=param_type)), value)
        self.assertTrue(response.success)
    ...
    def test_get_parameter_not_existing(self, param_type, service_type, service_request_type, default_value):
        rospy.wait_for_service('/march/parameter_server/get_param_{param_type}'.format(param_type=param_type))

        service = rospy.ServiceProxy('/march/parameter_server/get_param_{param_type}'.format(param_type=param_type),
                                     service_type)
        response = service.call(service_request_type('/not_existing_param_{param_type}'.format(param_type=param_type)))
