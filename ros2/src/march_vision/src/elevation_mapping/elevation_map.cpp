...
ElevationMap::ElevationMap(std::shared_ptr<rclcpp::Node> nodeHandle)
  ...
  RCLCPP_INFO(nodeHandle_->get_logger(), "Elevation map class initialized.");
  // TODO: Align type with updatedTimes. Here initialTime_ is RCL_ROS_TIME
...
bool ElevationMap::update(const grid_map::Matrix& varianceUpdate, const grid_map::Matrix& horizontalVarianceUpdateX,
  ...
  RCLCPP_INFO(nodeHandle_->get_logger(), "Entered method update() in map_.");
  ...
  RCLCPP_INFO(nodeHandle_->get_logger(), "Raw map has been updated with a new update matrix. (update() method)");
...
void ElevationMap::visibilityCleanup(const rclcpp::Time& updatedTime) {
  ...
  const auto methodStartTime = std::chrono::system_clock::now();
  // TODO: This is where the runtime error is
  //RCLCPP_INFO(nodeHandle_->get_logger(), "The value of updatedTime in vcl is %f", updatedTime.seconds());
  //RCLCPP_INFO(nodeHandle_->get_logger(), "The value of initialTime_ in vcl is %f", initialTime_.seconds());
  ...
  for (grid_map::GridMapIterator iterator(visibilityCleanupMap_); !iterator.isPastEnd(); ++iterator) {
    ...
    //RCLCPP_INFO(nodeHandle_->get_logger(), "Entered height layer loop.");
...
bool ElevationMap::postprocessAndPublishRawElevationMap() {
  ...
  RCLCPP_INFO(nodeHandle_->get_logger(), "Entered method postprocessAndPublishRawElevationMap().");
...
bool ElevationMap::publishFusedElevationMap() {
  ...
  RCLCPP_INFO(nodeHandle_->get_logger(), "Entered method publishFusedElevationMap().");
...
bool ElevationMap::publishVisibilityCleanupMap() {
  ...
  RCLCPP_INFO(nodeHandle_->get_logger(), "Entered method publishVisibilityCleanupMap().");
...
void ElevationMap::setTimestamp(rclcpp::Time timestamp) {
  RCLCPP_INFO_STREAM(nodeHandle_->get_logger(), "Setting timestamp: " << timestamp.nanoseconds() << " (type: " << typeid(timestamp).name() << ")");
