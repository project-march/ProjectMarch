...
int WirelessMaster::init()
{
    ...
    XsPortInfoArray::const_iterator wireless_master_port
        = detected_devices.begin();

    while (wireless_master_port != detected_devices.end()
        && !wireless_master_port->deviceId().isWirelessMaster()) {
    ...
    ROS_DEBUG("Found a device with ID: %s @ port: %s, baudrate: %d",
        ...
        wireless_master_port->portName().toStdString().c_str(),
        wireless_master_port->baudrate());

    if (!this->control_->openPort(
            wireless_master_port->portName().toStdString(),
            wireless_master_port->baudrate())) {
...
int WirelessMaster::configure(const int update_rate, const int channel)
{
    ...
    const XsIntArray supported_update_rates
        = this->master_->supportedUpdateRates();
    ...
    const int new_update_rate
        = findClosestUpdateRate(supported_update_rates, update_rate);
    ...
    ROS_DEBUG_STREAM(
        "Setting radio channel to " << channel << " and enabling radio...");
...
void WirelessMaster::waitForConnections(const size_t connections)
{
    ...
    auto has_connections = [this, connections] {
        return this->connected_mtws_.size() == connections;
    };
...
int WirelessMaster::findClosestUpdateRate(
    const XsIntArray& supported_update_rates, const int desired_update_rate)
{
