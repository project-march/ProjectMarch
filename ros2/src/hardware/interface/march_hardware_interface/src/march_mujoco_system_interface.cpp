...
#include "march_hardware_interface/march_mujoco_system_interface.hpp"
...
MarchMujocoSystemInterface::MarchMujocoSystemInterface()
 ...
 */
hardware_interface::return_type MarchMujocoSystemInterface::configure(const hardware_interface::HardwareInfo& info)
{
    RCLCPP_INFO((*logger_), "Configuring Mujoco Hardware Interface...");

    // Default Check needs to be done for every hardware interface.
    if (configure_default(info) != hardware_interface::return_type::OK) {
        RCLCPP_FATAL((*logger_), "Configure default of Mujoco Hardware Interface failed.");
 ...
 */
JointInfo MarchMujocoSystemInterface::build_joint_info(const hardware_interface::ComponentInfo& joint)
{
 ...
 */
std::vector<hardware_interface::StateInterface> MarchMujocoSystemInterface::export_state_interfaces()
{
 ...
 * will say that the joints are in the exact positions position controller wants them to be.
 */
std::vector<hardware_interface::CommandInterface> MarchMujocoSystemInterface::export_command_interfaces()
{
...
hardware_interface::return_type MarchMujocoSystemInterface::start()
{
    try {
 ...
 */
hardware_interface::return_type MarchMujocoSystemInterface::perform_command_mode_switch(
...
void MarchMujocoSystemInterface::make_joints_operational(std::vector<march::Joint*> joints)
{
    if (joints.empty()) {
        return;
    }
...
hardware_interface::return_type MarchMujocoSystemInterface::stop()
{
 ...
 */
hardware_interface::return_type MarchMujocoSystemInterface::read()
{
 ...
 */
void MarchMujocoSystemInterface::pdb_read()
{
 ...
 *  See: export_state_interfaces and export_command_interfaces().
 */
hardware_interface::return_type MarchMujocoSystemInterface::write()
{
...
bool MarchMujocoSystemInterface::is_joint_in_valid_state(JointInfo& jointInfo)
{
    return is_motor_controller_in_a_valid_state(jointInfo.joint, (*logger_)) && !is_joint_in_limit(jointInfo);
}

bool MarchMujocoSystemInterface::is_joint_in_limit(JointInfo& jointInfo)
{
    // SOFT Limit check.
...
std::unique_ptr<march::MarchRobot> MarchMujocoSystemInterface::load_march_hardware(
...
bool MarchMujocoSystemInterface::has_correct_actuation_mode(march::Joint& joint) const
{
...
PLUGINLIB_EXPORT_CLASS(march_hardware_interface::MarchMujocoSystemInterface, hardware_interface::SystemInterface)
