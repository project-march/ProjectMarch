...
class Sounds(object):
    def __init__(self, sound_names=None):
        ...
        self._sounds_dir = os.path.join(
            roslib.packages.get_pkg_dir("march_gait_selection"), "sounds"
        )
    ...
    def add_sound(self, sound):
        ...
        if sound in self._sounds:
            ...
            self._sounds[sound] = self._sound_client.waveSound(
                os.path.join(self._sounds_dir, "{0}.wav".format(sound))
            )
