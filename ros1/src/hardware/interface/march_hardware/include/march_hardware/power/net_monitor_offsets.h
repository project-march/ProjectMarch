...
#ifndef MARCH_HARDWARE_NET_MONITOR_OFFSETS_H
#define MARCH_HARDWARE_NET_MONITOR_OFFSETS_H
...
public:
    NetMonitorOffsets(int powerDistributionBoardCurrentByteOffset,
        ...
        int highVoltageOvercurrentTriggerByteOffset, int highVoltageEnabled,
    ...
    {
        if (powerDistributionBoardCurrent < 0 || lowVoltageNet1Current < 0
            || lowVoltageNet2Current < 0 || highVoltageNetCurrent < 0
            || lowVoltageState < 0 || highVoltageOvercurrentTrigger < 0
    ...
    int getLowVoltageNetCurrent(int netNumber) const
    {
        if (netNumber == 1) {
            ...
            ROS_FATAL("Can't get the byte offset for low voltage net current "
                      "%d, there are only 2 low voltage nets",
                netNumber);
            throw std::runtime_error(
                "Can't get the byte offset for low voltage net beside 1 and 2");
        }
    }
    ...
    friend bool operator==(
        const NetMonitorOffsets& lhs, const NetMonitorOffsets& rhs)
    {
        return lhs.powerDistributionBoardCurrent
            == rhs.powerDistributionBoardCurrent
            ...
            && lhs.lowVoltageState == rhs.lowVoltageState
            && lhs.highVoltageOvercurrentTrigger
            == rhs.highVoltageOvercurrentTrigger
            && lhs.highVoltageEnabled == rhs.highVoltageEnabled
            && lhs.highVoltageState == rhs.highVoltageState;
    }

    /** @brief Override stream operator for clean printing */
    friend ::std::ostream& operator<<(
        std::ostream& os, const NetMonitorOffsets& netMonitorOffsets)
    {
        return os << "NetMonitorOffsets(powerDistributionBoardCurrent: "
                  << netMonitorOffsets.powerDistributionBoardCurrent << ", "
                  << "lowVoltageNet1Current: "
                  << netMonitorOffsets.lowVoltageNet1Current << ", "
                  << "lowVoltageNet2Current: "
                  << netMonitorOffsets.lowVoltageNet2Current << ", "
                  << "highVoltageNetCurrent: "
                  << netMonitorOffsets.highVoltageNetCurrent << ", "
                  << "lowVoltageState: " << netMonitorOffsets.lowVoltageState
                  << ", "
                  << "highVoltageOvercurrentTrigger: "
                  << netMonitorOffsets.highVoltageOvercurrentTrigger << ", "
                  << "highVoltageEnabled: "
                  << netMonitorOffsets.highVoltageEnabled << ", "
                  << "highVoltageState: " << netMonitorOffsets.highVoltageState
                  << ")";
    }
};

#endif // MARCH_HARDWARE_NET_MONITOR_OFFSETS_H
