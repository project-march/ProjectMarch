...
#ifndef MARCH_HARDWARE_NET_DRIVER_OFFSETS_H
#define MARCH_HARDWARE_NET_DRIVER_OFFSETS_H
...
public:
    ...
    {
        if (lowVoltageNetOnOff < 0 || highVoltageNetOnOff < 0
            || highVoltageNetEnableDisable < 0) {
    ...
    friend bool operator==(
        const NetDriverOffsets& lhs, const NetDriverOffsets& rhs)
    {
        return lhs.lowVoltageNetOnOff == rhs.lowVoltageNetOnOff
            && lhs.highVoltageNetOnOff == rhs.highVoltageNetOnOff
            && lhs.highVoltageNetEnableDisable
            == rhs.highVoltageNetEnableDisable;
    }

    /** @brief Override stream operator for clean printing */
    friend ::std::ostream& operator<<(
        std::ostream& os, const NetDriverOffsets& netDriverOffsets)
    {
        return os << "NetDriverOffsets(lowVoltageNetOnOff: "
                  << netDriverOffsets.lowVoltageNetOnOff << ", "
                  << "highVoltageNetOnOff: "
                  << netDriverOffsets.highVoltageNetOnOff << ", "
                  << "highVoltageNetEnableDisable: "
                  << netDriverOffsets.highVoltageNetEnableDisable << ")";
    }
};

#endif // MARCH_HARDWARE_NET_DRIVER_OFFSETS_H
