...
namespace error {
    const char* getErrorDescription(ErrorType type)
    {
        switch (type) {
            ...
            case ErrorType::INVALID_ENCODER_COUNTS_PER_ROTATION:
                return "The encoder CPR (counts per rotation) is outside the allowed range";
            ...
            case ErrorType::INVALID_MOTOR_CONTROLLER:
                return "MotorController must be ODrive";
