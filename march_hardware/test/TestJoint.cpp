...
TEST_F(JointTest, TestReadEncodersOnce)
{
  ...
  double new_absolute_position = initial_absolute_position + velocity * elapsed_time.toSec() - 2 * this->imc->getAbsoluteRadPerBit();
...
TEST_F(JointTest, TestReadEncodersTwice)
{
  ...
  double second_absolute_position = initial_absolute_position + first_velocity * elapsed_time.toSec() - this->imc->getAbsoluteRadPerBit();
  double third_incremental_position = second_incremental_position + second_velocity * elapsed_time.toSec();
  double third_absolute_position = third_incremental_position + second_velocity * elapsed_time.toSec() - this->imc->getAbsoluteRadPerBit();
