...
void RobotNode::expressKinematics()
{
    ...
    for (unsigned int i = 0; i < global_position.rows(); i++)
    {
        ...
        ss_position_expression << "Global position " << m_name << " " << i << ": " << global_position(i, 0) << " ";
    ...
    for (unsigned int i = 0; i < global_rotation.rows(); i++)
    {
        for (unsigned int j = 0; j < global_rotation.cols(); j++)
        {
            ...
            ss_rotation_expression << "Global rotation " << m_name << " (" << i << "," << j << ") : " << global_rotation(i, j) << " ";
...
GiNaC::matrix RobotNode::expressGlobalPosition() const
{
    ...
    if (m_parent != nullptr)
    {
        GiNaC::matrix rotated_origin_position = m_parent->getOriginRotation().mul(global_position);
