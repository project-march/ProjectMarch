 ...
 *          If there are no stamps yet, an information message is printed to the screen and the function is exited.
 *          Then for every input device connected, their last_alive_stamp is checked.
 *          If there were connections before checking and are no connections after checking, then all input devices
 *          are lost and a non fatal error is published.
 */
void InputDeviceSafety::update()
{
  if (last_alive_stamps_.empty())
  {
    ...
    RCLCPP_INFO_THROTTLE(node_->get_logger(), *node_->get_clock(), THROTTLE_DURATION_MS, "No input device connected yet");
    return;
  }
  ...
  if (!has_connections)
  {
    RCLCPP_INFO_SKIPFIRST_THROTTLE(node_->get_logger(), *node_->get_clock(), THROTTLE_DURATION_MS, "No input device connected");
  }
}
 ...
 * @details If the input device is connected but timed_out, then the input device is actually no longer connected.
 *          The input device is then removed from the connected_devices.
 *
 *          If the input device is not connected ye and also not timed out, then a new input device is connected and
 *          the input device is added to the set of connected_devices
 */
void InputDeviceSafety::check_last_alive_stamp(const std::string& id, const rclcpp::Time& last_alive)
{
  auto now = node_->get_clock()->now();

  const bool timed_out = now > (last_alive + connection_timeout_);
  const bool is_connected = connected_devices_.find(id) != connected_devices_.end();
  ...
  {
    RCLCPP_WARN_STREAM_THROTTLE(node_->get_logger(), *node_->get_clock(), THROTTLE_DURATION_MS, "Input device `" << id << "` alive message is from the future. Current time is "
