...
protected:
    ...
    const int32_t zero_position = lower_limit - lower_limit_rad * counts_per_rotation / (2 * M_PI);
    ...
    const int32_t lower_soft_limit = lower_soft_limit_rad * counts_per_rotation / (2 * M_PI) + zero_position;
    const int32_t upper_soft_limit = upper_soft_limit_rad * counts_per_rotation / (2 * M_PI) + zero_position;

    march::AbsoluteEncoder encoder = march::AbsoluteEncoder(counts_per_rotation, motor_controller_type, lower_limit,
        upper_limit, lower_limit_rad, upper_limit_rad, lower_soft_limit_rad, upper_soft_limit_rad);
};
...
{
    ASSERT_THROW(
        march::AbsoluteEncoder(this->counts_per_rotation, motor_controller_type, this->lower_limit, this->upper_limit,
            this->lower_limit_rad, this->upper_limit_rad, this->upper_soft_limit_rad, this->lower_soft_limit_rad),
        march::error::HardwareException);
}

TEST_F(AbsoluteEncoderTest, LowerSoftLimitLowerThanLowerHardLimit)
{
    ASSERT_THROW(march::AbsoluteEncoder(this->counts_per_rotation, motor_controller_type, this->lower_limit,
                     this->upper_limit, this->lower_limit_rad, this->upper_limit_rad, -0.4, this->upper_soft_limit_rad),
        march::error::HardwareException);
}
...
{
    ASSERT_THROW(march::AbsoluteEncoder(this->counts_per_rotation, motor_controller_type, this->lower_limit,
                     this->upper_limit, this->lower_limit_rad, this->upper_limit_rad, this->lower_soft_limit_rad, 2.0),
        march::error::HardwareException);
}

TEST_F(AbsoluteEncoderTest, ZeroPositionRadToZeroPosition)
{
    ASSERT_EQ(this->encoder.positionRadiansToIU(/*position=*/0.0), this->zero_position);
}

TEST_F(AbsoluteEncoderTest, CorrectFromRad)
{
    const double radians = 1.0;
    const int32_t expected = (radians * this->counts_per_rotation / (2 * M_PI)) + this->zero_position;
    ASSERT_EQ((int32_t)this->encoder.positionRadiansToIU(radians), expected);
}

TEST_F(AbsoluteEncoderTest, ZeroPositionToZeroRadians)
{
    ASSERT_DOUBLE_EQ(this->encoder.positionIUToRadians(this->zero_position), 0.0);
}

TEST_F(AbsoluteEncoderTest, CorrectToRad)
{
    const int32_t iu = 1.0;
    const double expected = (iu - this->zero_position) * 2 * M_PI / this->counts_per_rotation;
