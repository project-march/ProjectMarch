...
class MpcListener(Node):
    ...
    def set_lengths(self) -> None:
        ...
        self.future_reference_position = np.empty(shape=(self.number_of_joints, self.future_time_steps), dtype=float)
        self.future_reference_velocity = np.empty(shape=(self.number_of_joints, self.future_time_steps), dtype=float)
        self.future_reference_input = np.empty(shape=(self.number_of_joints, self.future_time_steps), dtype=float)
    ...
    def stream_estimation(self) -> Response:
        ...
        for joint_number in range(self.number_of_joints):
            ...
            self.response_body_estimation[f"joint_{joint_number}_reference_position"] = self.future_reference_position[
                joint_number, :
            ].tolist()
            self.response_body_estimation[f"joint_{joint_number}_estimation_velocity"] = self.new_estimation_velocity[
                joint_number, :
            ].tolist()
            self.response_body_estimation[f"joint_{joint_number}_reference_velocity"] = self.future_reference_velocity[
                joint_number, :
            ].tolist()
            self.response_body_estimation[f"joint_{joint_number}_estimation_input"] = self.new_estimation_input[
                joint_number, :
            ].tolist()
            self.response_body_estimation[f"joint_{joint_number}_reference_input"] = self.future_reference_input[
                joint_number, :
            ].tolist()
