...
class SubgaitTest(unittest.TestCase):
    ...
    def load_interpolatable_subgaits(
        ...
        Args:
          subgait_name: (Default value = "left_close")
          base_version: (Default value = "MV_walk_leftclose_v1")
          other_version: (Default value = "MV_walk_leftclose_v2")
    ...
    def load_interpolatable_subgaits_ik(
        ...
        Args:
          subgait_name: (Default value = "swing")
          base_version: (Default value = "forward_swing")
          other_version: (Default value = "backward_swing")
        ...
        for i, joint in enumerate(new_subgait.joints):
            for j, setpoint in enumerate(joint.setpoints):
                ...
                self.assertAlmostEqual(
                    base_setpoint.time * (1 - parameter) + parameter * other_setpoint.time,
                    setpoint.time,
                    places=4,
                )
                self.assertAlmostEqual(
                    base_setpoint.position * (1 - parameter) + parameter * other_setpoint.position,
                    setpoint.position,
                    places=4,
                )
                self.assertAlmostEqual(
                    base_setpoint.velocity * (1 - parameter) + parameter * other_setpoint.velocity,
                    setpoint.velocity,
                    places=4,
                )
