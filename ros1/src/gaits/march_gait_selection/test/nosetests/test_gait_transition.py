...
class TestTransitionTrajectory(unittest.TestCase):
    def setUp(self):
        ...
        self.resources = os.path.join(
            rospkg.RosPack().get_path(VALID_PACKAGE), VALID_DIRECTORY
        )

    def test_walk_transition_small_to_medium_right_swing(self):
        #  Test if the TransitionSubgait is created without an error
        walk_small = Subgait.from_name_and_version(
            self.robot, self.resources, "walk_small", "right_swing", "MIV_final"
        )
    ...
    def test_transition_walk_small_stairs_up_right_swing(self):
        ...
        stairs_up = Subgait.from_name_and_version(
            self.robot, self.resources, "stairs_up", "right_swing", "MIV_final"
        )
