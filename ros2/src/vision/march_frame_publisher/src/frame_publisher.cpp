            ...
            /*automatically_declare_parameters_from_overrides=*/true))
{
    ...
    callback_handle_ = this->add_on_set_parameters_callback(std::bind(
        &FramePublisher::parametersCallback, this, std::placeholders::_1));

    align_service_
        = this->create_service<std_srvs::srv::Trigger>("~/align_cameras",
            std::bind(&FramePublisher::alignCameras, this,
...
void FramePublisher::publishCameraFrame(const std::string& left_or_right)
{
    ...
    if ((left_or_right == "left"
        ...
        || (left_or_right == "right"
            && rclcpp::Time(tr.header.stamp) > last_published_camera_right_)) {
        tr.header.frame_id
