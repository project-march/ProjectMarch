...
namespace march
{
LowVoltage::LowVoltage(PdoSlaveInterface& pdo, NetMonitorOffsets netMonitoringOffsets,
...
{
  bit32 current = this->pdo_.read32(this->netMonitoringOffsets.getLowVoltageNetCurrent(netNumber));
  return current.f;
}
...
bool LowVoltage::getNetOperational(int netNumber)
{
  ...
  bit8 operational = this->pdo_.read8(this->netMonitoringOffsets.getLowVoltageState());
