...
ObstacleController::ObstacleController(physics::ModelPtr model, std::map<std::string, int>& pd_values)
...

{
    ...
    std::string path = ament_index_cpp::get_package_share_directory("march_description")
        + "/urdf/properties/properties_" + model_->GetName() + ".yaml";
    if (!file_exists(path)) {
        std::cerr << "ERROR: Can't find file for gazebo: " << path << std::endl;
    }
    YAML::Node properties = YAML::LoadFile(path);
    auto upper_leg_length_ = properties["dimensions"]["upper_leg"]["length"].as<double>();
...
void ObstacleController::newSubgait(const march_shared_msgs::msg::CurrentGait::ConstSharedPtr& msg)
{
    if (subgait_name_ == "right_open" or subgait_name_ == "right_swing" or subgait_name_ == "left_swing") {
        // Exponential smoothing with alpha = 0.8
        double alpha = 0.8;
        swing_step_size_ = alpha * swing_step_size_
            + (1 - alpha) * 2 * std::abs(foot_right_->WorldPose().Pos().X() - foot_left_->WorldPose().Pos().X());
    }
...
void ObstacleController::update(
    ignition::math::v6::Vector3<double>& torque_left, ignition::math::v6::Vector3<double>& torque_right)
{
    ...
    double T_pitch = -pd_values_["p_pitch"] * error_x - pd_values_["d_pitch"] * (error_x - error_x_last_timestep_);
    double T_roll = pd_values_["p_roll"] * error_y + pd_values_["d_roll"] * (error_y - error_y_last_timestep_);
    double T_yaw = -pd_values_["p_yaw"] * error_yaw - pd_values_["d_yaw"] * (error_yaw - error_yaw_last_timestep_);
...
void ObstacleController::getGoalPosition(double time_since_start)
{
    ...
    if (subgait_name_.find(/*__s=*/"freeze") == std::string::npos) {
        if (subgait_name_.find(/*__s=*/"sit") != std::string::npos || subgait_name_ == "prepare_stand_up") {
