    ...
    const joint_trajectory_controller::State& state_error)
{
  ...
  for (unsigned int i = 0; i < num_joints_; i++)
  {
    ...
    current_state = { (*joint_handles_ptr_)[i].getPosition(), (*joint_handles_ptr_)[i].getVelocity() };
    model_predictive_controllers_[i].setInitialState(current_state);

    // Set the reference
    for (unsigned int j = 0; j < desired_states.size(); j++)
    {
      model_predictive_controllers_[i].setReferenceI(
          j, { desired_states[j].position[i], desired_states[j].velocity[i], 0.0 });
    }
    //
    model_predictive_controllers_[i].setReferenceEnd({ desired_states[desired_states.size() - 1].position[i],
                                                       desired_states[desired_states.size() - 1].velocity[i] });
