...
YAML::Node RealSenseReader::getConfigIfPresent(std::string key) {
  if (config_tree_[key]) {
    ...
    ROS_WARN_STREAM("Key " << key << " was not found in the config "
                                                   "file, empty config will be used");
    return YAML::Node();
  }
}
...
void RealSenseReader::pointcloud_callback(const PointCloud::ConstPtr& input_cloud)
{
  if (reading_)
  {
    ...
    std::unique_ptr<SimplePreprocessor> preprocessor =
        std::make_unique<SimplePreprocessor>(getConfigIfPresent("preprocessor"),
                                             pointcloud,
                                             normals);
    ...
    std::unique_ptr<SimpleRegionCreator> region_creator =
        std::make_unique<SimpleRegionCreator>(getConfigIfPresent("region_creator"),
                                              pointcloud,
                                              normals,
                                              regions_vector);
    region_creator->create_regions();
  }
}
