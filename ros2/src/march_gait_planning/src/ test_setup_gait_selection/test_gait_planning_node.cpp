...
TestGaitPlanningNode::TestGaitPlanningNode()
 : Node("march_test_gait_planning_node"), 
   ...
   //TODO: Change this to the FootPositions msg we are going to use
   ...
   m_response_received(true)
 {
    m_exo_state_subscriber = create_subscription<march_shared_msgs::msg::ExoState>(
        "current_state", 10, std::bind(&Test::currentStateCallback, this, _1)); 
    
    m_test_joint_trajectory_controller_state_pub_ = this->create_publisher<control_msgs::msg::JointTrajectoryControllerState>(
    ...
    auto timer_callback = std::bind(&Test::timerCallback, this);
...
void Test::currentStanceFootCallback(const std_msgs::msg::Int32::SharedPtr msg){
    RCLCPP_INFO(get_logger(), "Received current stance foot: %d", msg->data); 
    m_gait_planning.setStanceFoot(msg->data); 
}

void Test::currentFeetPositionsCallback(const march_shared_msgs::msg::IksFootPositions::SharedPtr msg){
...
void Test::responseStanceLegCallback(
...
void Test::sendRequest(const bool& gait_complete){
    if (gait_complete){
        ...
        auto future = m_stance_leg_client->async_send_request(m_stance_leg_request, std::bind(&Test::responseStanceLegCallback, this, _1)); 
...
void Test::footPositionsPublish(){
    if (m_gait_planning.getGaitType() == exoState::Stand){
        ...
        RCLCPP_INFO(rclcpp::get_logger("march_test_gait_planning"), "Home stand position published!");
    }
        ...
        RCLCPP_INFO(rclcpp::get_logger("march_test_gait_planning"), "Current stance foot is= %d", m_gait_planning.getCurrentStanceFoot());
