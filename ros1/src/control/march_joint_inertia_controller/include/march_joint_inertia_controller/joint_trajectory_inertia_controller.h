...
public:
     ...
     * joints, the pid calculators and the inertia estimators
     */
        ...
        ros::NodeHandle& nh)
    {
        ...
        for (unsigned int i = 0; i < pids_.size(); ++i) {
            ...
            if (!pids_[i]->init(joint_nh)) {
                ROS_WARN_STREAM("Failed to initialize PID gains from ROS "
                                "parameter server.");
                return false;
            }
        }
        ...
        std::vector<std::vector<double>> vibration_boundaries { { 0.0, 0.0 },
            { 0.0, 0.0 } };
        ...
        rotary_estimator_nh.param(
            "alpha_filter_size", alpha_filter_size[0], 12);
        rotary_estimator_nh.param(
        ...
        linear_estimator_nh.param(
            "vibration_boundaries", vibration_boundaries[1], default_vibration);

        // Initialize the estimator parameters
        inertia_estimators_.resize(num_joints_);
        for (unsigned int j = 0; j < num_joints_; ++j) {
            inertia_estimators_[j].setLambda(
                lambda[(int)floor(j / 2) % 2]); // Produce sequence 00110011
            inertia_estimators_[j].setAccSize(
                alpha_filter_size[(int)floor(j / 2) % 2]);
            inertia_estimators_[j].setVibrationBoundaries(
                vibration_boundaries[(int)floor(j / 2) % 2]);
            //      pub_[j] =
            //      nh.advertise<std_msgs::Float64>("/inertia_publisher/" +
            //      joint_handles[j].getName(), 100);
            this->pub_[j] = std::make_unique<
    ...
    void updateCommand(const ros::Time& /*time*/, const ros::Duration& period,
        const std::vector<
            joint_trajectory_controller::State>& /*desired state*/,
        const joint_trajectory_controller::State& state_error)
    {
        ...
        if (count_ < samples_) {
            ...
            for (unsigned int i = 0; i < num_joints_; ++i) {
                inertia_estimators_[i].fillBuffers(
                    (*joint_handles_ptr_)[i].getVelocity(),
                    (*joint_handles_ptr_)[i].getEffort(), period);
                inertia_estimators_[i].standard_deviation.push_back(
                    inertia_estimators_[i].getAcceleration());
                if (count_ == samples_) {
            ...
            for (unsigned int i = 0; i < num_joints_; ++i) {
                ...
                // Update PIDs
                const double command = (pids_[i]->computeCommand(
     ...
     * filled, resets the pid calculators and sets the intial command to zero so
     * that the joint doesn't start moving without a desired trajectory
     */
    ...
    /**
     * \brief No specific implementation needed I guess, but if necessary later
     * then here it is.
     */
     ...
     */
    void getGains(int joint_nr, double& p, double& i, double& d, double& i_max,
        double& i_min)
    {
        bool dummy;
     ...
     */
    void setGains(int joint_nr, const double& p, const double& i,
