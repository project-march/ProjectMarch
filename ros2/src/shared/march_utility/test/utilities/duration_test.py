...
class TestTransitionTrajectory(unittest.TestCase):
    ...
    def test_get_nanoseconds(self):
        duration = CustomDuration(seconds=1, nanoseconds=500000000)
        self.assertEqual(duration.nanoseconds, 1500000000)

    def test_get_seconds_from_nanoseconds(self):
        duration = CustomDuration(nanoseconds=1000000000)
        self.assertAlmostEqual(duration.seconds, 1)

    def test_get_seconds(self):
        duration = CustomDuration(seconds=1)
        self.assertAlmostEqual(duration.seconds, 1)

    def test_add(self):
        duration_1 = CustomDuration(seconds=1)
        duration_2 = CustomDuration(seconds=3)
        self.assertEqual((duration_1 + duration_2).nanoseconds, 4000000000)

    def test_sub(self):
        duration_1 = CustomDuration(seconds=3)
        duration_2 = CustomDuration(seconds=1)
