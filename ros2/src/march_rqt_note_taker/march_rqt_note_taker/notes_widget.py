...
class NotesWidget(QWidget):
        ...
        if self._has_autosave and self._last_save_file is not None:
            if self._autosave_file is None or self._autosave_file.closed:
                try:
                    ...
                    self._node.get_logger().error('Failed to open file {f} for autosaving: {e}'.
                                                  format(f=self._last_save_file, e=e))
                    return
            ...
            try:
                if action == self.INSERT:
                    ...
                    self._node.get_logger().warn('Unknown autosave action: {a}'.format(a=action))
                    return
                self._autosave_file.flush()
            except IOError as e:
                self._node.get_logger().error('Failed to write to file {f} for autosaving: {e}'.
                                              format(f=self._last_save_file, e=e))
            else:
    ...
    def _save(self, file_name: str):
        ...
        try:
            ...
            self._node.get_logger().error('Failed to open file: {e}'.format(e=e))
        else:
            self._set_saved(True)
