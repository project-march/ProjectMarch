...
bool MarchHardwareInterface::init(ros::NodeHandle& nh, ros::NodeHandle& /* robot_hw_nh */)
{
    ...
    power_distribution_board_data_pub_
        = std::make_unique<realtime_tools::RealtimePublisher<march_shared_msgs::PowerDistributionBoardData>>(
            nh, "/march/pdb_data/", 4);
    ...
    for (size_t i = 0; i < num_joints_; ++i) {
        ...
        ROS_DEBUG("[%s] ROS soft limits set to (%f, %f) and error limits set "
                  "to (%f, %f)",
            name.c_str(), soft_limits.min_position, soft_limits.max_position, soft_limits_error.min_position,
    ...
    for (size_t i = 0; i < num_joints_; ++i) {
        ...
        JointStateHandle joint_state_handle(
            joint.getName(), &joint_position_[i], &joint_velocity_[i], &joint_effort_[i]);
        ...
        MarchTemperatureSensorHandle temperature_sensor_handle(
            joint.getName(), &joint_temperature_[i], &joint_temperature_variance_[i]);
    ...
    for (size_t i = 0; i < num_joints_; ++i) {
        ...
        if (!is_operational[i]) {
            ...
            if (joint_wait_duration.has_value()) {
                wait_duration = std::max(joint_wait_duration.value(), wait_duration);
            }
        }
    }
...
void MarchHardwareInterface::call_and_wait_while_checking_for_each_joint(std::function<bool(march::Joint&)> const& f,
    std::optional<std::function<void(march::Joint&)>> const& maximum_tries_debug_f = std::nullopt,
    const ros::Duration wait_duration = ros::Duration(/*t=*/1), const unsigned maximum_tries = 10)
{
...
void MarchHardwareInterface::startJoints()
{
    ...
    call_and_wait_while_checking_for_each_joint(
        ...
        [](march::Joint& joint) {
            ROS_INFO("Joints %s is not receiving data", joint.getName().c_str());
        });
    ...
    if (!(all(is_operational))) {
        ...
        call_and_wait_while_checking_for_each_joint(
            ...
            [](march::Joint& joint) {
                ROS_INFO("Joints %s is not in operational state", joint.getName().c_str());
            });
    }
...
void MarchHardwareInterface::read(const ros::Time& /* time */, const ros::Duration& elapsed_time)
{
    for (size_t i = 0; i < num_joints_; i++) {
        ...
        ROS_INFO_STREAM("Joint " << joint.getName() << ", position= " << joint_position_[i]
...
void MarchHardwareInterface::write(const ros::Time& /*time*/, const ros::Duration& elapsed_time)
{
    for (size_t i = 0; i < num_joints_; i++) {
        ...
        joint_effort_command_[i]
            = joint_effort_command_[i] * march_robot_->getJoint(i).getMotorController()->effortMultiplicationConstant();
        ...
        auto effort_limit = march_robot_->getJoint(i).getMotorController()->getEffortLimit();
        joint_effort_command_[i] = std::clamp(joint_effort_command_[i], -effort_limit, effort_limit);
    ...
    if (not has_actuated_) {
        ...
        for (size_t i = 0; i < num_joints_; i++) {
            if (joint_effort_command_[i] != 0) {
                ROS_ERROR(
                    "Non-zero effort on first actuation for joint %s", march_robot_->getJoint(i).getName().c_str());
                found_non_zero = true;
            }
        }
        if (found_non_zero) {
...
void MarchHardwareInterface::reserveMemory()
{
    ...
    motor_controller_state_pub_->msg_.motor_temperature.resize(num_joints_);
    motor_controller_state_pub_->msg_.fet_temperature.resize(num_joints_);
...
void MarchHardwareInterface::updateMotorControllerState()
{
    ...
    for (size_t i = 0; i < num_joints_; i++) {
        ...
        motor_controller_state_pub_->msg_.fet_temperature[i] = motor_controller_state->fet_temperature_;
        motor_controller_state_pub_->msg_.motor_temperature[i] = motor_controller_state->motor_temperature_;
...
void MarchHardwareInterface::outsideLimitsCheck(size_t joint_index)
{
    ...
    if (joint_position_[joint_index] < soft_limits_[joint_index].min_position
        ...
        if (joint_position_[joint_index] < soft_limits_error_[joint_index].min_position
            ...
            if (joint.getName() == "left_ankle" || joint.getName() == "right_ankle") {
            ...
            if (soft_limit_touched_[joint_index]
                && ros::Time::now() - soft_limit_breach_times_[joint_index] >= soft_limit_timeout_) {
        ...
        ROS_WARN_THROTTLE(1,
            ...
            "%f",
            joint.getName().c_str(), soft_limits_[joint_index].min_position, soft_limits_[joint_index].max_position,
...
void MarchHardwareInterface::getSoftJointLimitsError(const std::string& name,
    const urdf::JointConstSharedPtr& urdf_joint, joint_limits_interface::SoftJointLimits& error_soft_limits)
{
...
void MarchHardwareInterface::updatePowerDistributionBoardData()
{
    ...
    march::PowerDistributionBoardData pdb_data = march_robot_->getPowerDistributionBoard().read();
