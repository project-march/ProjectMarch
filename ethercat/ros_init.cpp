...
//--//--------------------------------------------------------------------------------------------//
	...
	for(int i = 0; i < *LaunchParameters::get_number_of_joints(); i++)
	{
		...
		jointHandlerPubMap[prefix] = (
			new ros::Publisher(
				nh.advertise<custom_msgs::ECtoJH>(
					"ECto" + prefix, 
					50
				)
			)
	...
	//--------------------------------------------------------------------------------------------//
		...
		jointPositionSubList.push_back(
			new ros::Subscriber(
				nh.subscribe(
					prefix + "TargetPositionIU", 
					1, 
					&Imc_callback::set_target_position
				)
			)
		);
...
//--//--------------------------------------------------------------------------------------------//
	...
	for (vector<string>::iterator i = LaunchParameters::get_list_of_GES()->begin(); i != LaunchParameters::get_list_of_GES()->end(); ++i)
	{
		...
		else if(*i == "LLG" || *i == "RLG")
		{
			gesHandlerPubMap[*i] = (
				new ros::Publisher(
					nh.advertise<custom_msgs::ECtoLG>(
						"ECto" + *i, 
						50
					)
				)
			...
			speakerSubPtr = (
				new ros::Subscriber(
					nh.subscribe(
						*i + "toEC", 
						50, 
						&Backpack_ges_callback::send_frequency
					)
				)	
			);
...
//--//--------------------------------------------------------------------------------------------//
	...
	safetyPubPtr = (
		new ros::Publisher(
			nh.advertise<custom_msgs::data8Msg>(
				"ECErrorMessage", 
				50
			)
		)
...
//--//--------------------------------------------------------------------------------------------//
	...
	if (*LaunchParameters::get_number_of_Pdb())
	{
		PdbHandlerPubPtr = (
				new ros::Publisher(
					nh.advertise<custom_msgs::ECtoPDB>(
						"ECtoPDB", 
						50
				)
			)
...
//--//--------------------------------------------------------------------------------------------//
	...
	if (*LaunchParameters::get_number_of_ipd())
	{
		...
		IpdHandlerPubPtr = (
				new ros::Publisher(
					nh.advertise<custom_msgs::ECtoIPD>(
						"ECtoIPD", 
						50
					)
				)
