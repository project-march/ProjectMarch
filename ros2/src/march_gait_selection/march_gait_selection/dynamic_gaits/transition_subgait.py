...
class TransitionSubgait(Subgait):
    ...
    def __init__(self, joints, duration,
        ...
        super(TransitionSubgait, self).__init__(joints, duration, gait_type,
    ...
    def from_subgaits(cls, old_subgait, new_subgait, transition_subgait_name):
        ...
        transition_joints = cls._transition_joints(
            old_subgait_copy, new_subgait_copy)
        ...
        cls._validate_transition_gait(old_subgait_copy, transition_subgait,
                                      new_subgait_copy)
        cls._validate_transition_trajectory(old_subgait_copy,
                                            transition_subgait,
                                            new_subgait_copy)
            ...
            for old_setpoint, transition_setpoint, new_setpoint in zip(
                ...
                if old_setpoint.time != transition_setpoint.time:
                    raise TransitionError(
                        'The transition timestamp {tt} != the old timestamp {ot}'
                        .format(tt=transition_setpoint.time,
                                ot=old_setpoint.time))

                if new_setpoint.time != transition_setpoint.time:
                    raise TransitionError(
                        'The transition timestamp {tt} != the new timestamp {ot}'
                        .format(tt=transition_setpoint.time,
                                ot=new_setpoint.time))
