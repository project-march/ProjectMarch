...
def data_files():
    ...
    for file in iglob(os.path.join(test_gait_files_source, '**', '*.subgait'), recursive=True):
        data.append((os.path.join('share', package_name, 'test', 'resources',
                                  os.path.dirname(os.path.relpath(file, test_gait_files_source))), [file]))
    for file in iglob(os.path.join(test_gait_files_source, '**', '*.gait'), recursive=True):
        data.append((os.path.join('share', package_name, 'test', 'resources',
                                  os.path.dirname(os.path.relpath(file, test_gait_files_source))), [file]))
    return data
