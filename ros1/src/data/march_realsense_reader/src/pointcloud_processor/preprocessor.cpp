...

{
    ...
    try {
        ...
        ros::Time pointcloud_time(pointcloud_->header.stamp);
//        if (tfBuffer->canTransform(frame_id_to_transform_to_,
//                pointcloud_frame_id, pointcloud_time, ros::Duration(/*t=*/1.0))) {
        transform_stamped
            = tfBuffer->lookupTransform(frame_id_to_transform_to_,
                pointcloud_frame_id, pointcloud_time);
        ROS_DEBUG_STREAM("looking for transformation at time " << pointcloud_time);
