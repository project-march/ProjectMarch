...
WalkController::WalkController(physics::ModelPtr model) : ObstacleController(model)
{
  ...
  com_levels_tree = YAML::LoadFile(ros::package::getPath("march_gazebo_plugins") + "/config/com_levels.yaml");

  for (YAML::const_iterator it = com_levels_tree.begin(); it != com_levels_tree.end(); ++it) {
    std::string key = it->first.as<std::string>();
    com_levels.push_back(key);
  }
  // The non-balance_ values are tune so that the exo stays upright, the _balance_ are set so for balance.
  // These _balance_ values can be changed if one wishes the plugin to deliver some torque even when turned balance.
